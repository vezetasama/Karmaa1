...
function TopUpCard({ game, isGrid = false }) {
  const customTopupRoutes = {
    ...
    'discord-nitro': '/product/discord-nitro',
...
export default function TopUps({
  ...
  return (
    ...
    >
      ...
      {/* Carousel Layout - hide when both grids are enabled, hide on desktop when desktopGrid only, hide on mobile when isMobileGrid only */}
      <div className={`relative group/carousel overflow-hidden ${desktopGrid && isMobileGrid ? 'hidden' : desktopGrid ? 'md:hidden' : isMobileGrid ? 'hidden md:block' : ''}`}>
        {/* Left fade edge */}
        <div
