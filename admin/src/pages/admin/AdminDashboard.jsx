              ...
              {stats.recentOrders.map((o) => (
                <div key={o._id} className="flex items-center gap-3 p-2.5 rounded-lg hover:bg-dark-900/40 transition-colors">
                  ...
                  <div className="text-right">
                    ...
                    <p className={`text-[10px] font-medium ${o.status === 'completed' || o.status === 'delivered' ? 'text-green-400' : o.status === 'pending' ? 'text-yellow-400' : 'text-neon-cyan'}`}>
                      {o.status === 'delivered' ? 'completed' : o.status}
                    </p>
                  </div>
                </div>
              ))}
