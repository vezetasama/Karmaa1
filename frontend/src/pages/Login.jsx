...
export default function Login() {
  ...
  const handleGoogleError = useCallback((error) => {
    ...
    clearError();
    const msg = error?.type === 'popup_closed_by_user'
      ? 'Sign-in was cancelled.'
      : error?.type === 'access_denied'
        ? 'Access was denied.'
        : 'Google sign-in failed. Please try again.';
    useAuthStore.getState().set({ error: msg });
  }, [clearError]);
  ...
  useEffect(() => {
    ...
    const setup = () => {
      ...
      window.google.accounts.id.renderButton(el, {
        ...
        text: 'continue_with',
        shape: 'rectangular',
        width: Math.min(el.offsetWidth || 384, 400),
      });
      return true;
    };
  ...
  }, [handleGoogleResponse, handleGoogleError]);
          ...
          {GOOGLE_CLIENT_ID && (
            <>
              ...
              <div className="relative w-full group" style={{ height: '44px' }} id="login-google">
                <div className="absolute inset-0 flex items-center justify-center gap-3 rounded-xl border border-white/[0.1] bg-white/[0.03] group-hover:bg-white/[0.07] text-white font-medium text-sm transition-all duration-200 group-hover:border-white/[0.2] pointer-events-none">
