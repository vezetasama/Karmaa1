    ...
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-10 sm:py-12">
      ...
      <div className="mt-6 rounded-2xl border border-white/[0.06] bg-dark-800/40 backdrop-blur-sm p-6">
        ...
        <div className="flex flex-col sm:flex-row gap-3">
          <button onClick={handleAddToCart} disabled={isOutOfStock} className={`btn-secondary flex-1 flex items-center justify-center gap-2 !rounded-full ${isOutOfStock ? 'opacity-50 cursor-not-allowed border-white/[0.04] text-gray-500' : ''}`} id="pubg-lite-add-to-cart">Add to Cart</button>
          <button onClick={handleBuyNow} disabled={isOutOfStock} className={`btn-primary flex-1 flex items-center justify-center gap-2 ${isOutOfStock ? 'opacity-60 cursor-not-allowed bg-red-950/40 border-red-500/30 text-red-400/90 shadow-none hover:bg-red-950/40 hover:border-red-500/30' : ''}`} id="pubg-lite-buy-now">
            {isOutOfStock ? (
