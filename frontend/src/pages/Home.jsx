...
export default function Home() {
  ...
  const [pauseHeroAuto, setPauseHeroAuto] = useState(false);
  const heroDragRef = useRef({ active: false, startX: 0, pointerId: null });
  const suppressHeroLinkClickRef = useRef(false);

  const whyUsRef = useFadeIn();
  const testimonialsRef = useFadeIn();
  const ctaRef = useFadeIn();

  const HERO_DRAG_THRESHOLD = 56;
  ...
  useEffect(() => {
    if (pauseHeroAuto) return undefined;
    const timer = setInterval(nextSlide, 5000);
    return () => clearInterval(timer);
  }, [nextSlide, pauseHeroAuto]);

  const onHeroPointerDown = useCallback((e) => {
    if (e.pointerType === 'mouse' && e.button !== 0) return;
    heroDragRef.current = { active: true, startX: e.clientX, pointerId: e.pointerId };
    setPauseHeroAuto(true);
    e.currentTarget.setPointerCapture(e.pointerId);
  }, []);

  const finishHeroPointer = useCallback(
    (clientX, pointerId, currentTarget) => {
      const s = heroDragRef.current;
      if (!s.active || s.pointerId !== pointerId) return;
      heroDragRef.current = { active: false, startX: 0, pointerId: null };
      setPauseHeroAuto(false);
      try {
      ...
      if (Math.abs(dx) < HERO_DRAG_THRESHOLD) return;
      suppressHeroLinkClickRef.current = true;
      if (dx < 0) nextSlide();
      else prevSlide();
    },
    [nextSlide, prevSlide]
  ...
  const onHeroPointerUp = useCallback(
    (e) => {
      finishHeroPointer(e.clientX, e.pointerId, e.currentTarget);
    },
    [finishHeroPointer]
  );
  ...
  const onHeroLostPointerCapture = useCallback(() => {
    if (heroDragRef.current.active) {
      heroDragRef.current = { active: false, startX: 0, pointerId: null };
      setPauseHeroAuto(false);
    }
  }, []);
    ...
    <div className="pb-8 md:pb-12">
      ...
      <section className="relative" id="hero-slider">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 pt-4 sm:pt-6">
          <div
            className="relative rounded-2xl overflow-hidden h-[210px] sm:h-[280px] md:h-[420px] lg:h-[460px] border border-white/[0.06] touch-pan-y select-none cursor-grab active:cursor-grabbing"
            onPointerDown={onHeroPointerDown}
            onPointerUp={onHeroPointerUp}
            onPointerCancel={onHeroPointerCancel}
            onLostPointerCapture={onHeroLostPointerCapture}
            role="region"
            ...
            <div className="absolute inset-0 hidden md:flex items-end sm:items-center p-6 sm:p-8 md:p-12 lg:p-16 z-10">
              <div className="animate-fade-in max-w-xl" key={currentSlide}>
                ...
                <Link
                  ...
                  onClick={(e) => {
                    if (suppressHeroLinkClickRef.current) {
                      e.preventDefault();
                      suppressHeroLinkClickRef.current = false;
                    }
                  }}
                  className="inline-flex items-center gap-2 mt-4 sm:mt-6 px-6 sm:px-8 py-2.5 sm:py-3 rounded-full font-semibold text-white text-sm transition-all duration-300 hover:brightness-110 hover:scale-105 hover:-translate-y-0.5 active:scale-95 relative z-20"
                  style={{
