...
import { saveGuestOrderId } from '../utils/guestOrders';

const methodMeta = {
  esewa: { name: 'eSewa', qr: '/images/pay-esewa-qr.jpg' },
  khalti: { name: 'Khalti', qr: '/images/pay-khalti-qr.jpg' },
  bank: { name: 'Bank Transfer', qr: '/images/pay-esewa-qr.jpg' },
};
...
export default function CheckoutPayment() {
  ...
  /** Sync flag — React state updates too late if clearCart runs before re-render. */
  ...
  useEffect(() => {
    if (!paymentFinishedRef.current && items.length === 0) {
      navigate('/cart', { replace: true });
    }
  }, [items.length, navigate]);

  if (!paymentFinishedRef.current && items.length === 0) return null;
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      for (const item of items) {
        ...
        const productIdOrSlug = isObjectId(item.product?._id)
          ? item.product._id
          : slug;
        ...
        if (packageIndex < 0 || !matchedPackage) {
          throw new Error(
            `Package "${item.selectedPackage?.label || 'unknown'}" is not available for ${realProduct.name}. Remove it from your cart and add it again.`
          );
        }
        ...
        if (orderRes.data?.data?._id) {
          saveGuestOrderId(orderRes.data.data._id);
        }
      }

      paymentFinishedRef.current = true;
      showToast('Payment submitted! Your order is pending verification.', 'success');
      navigate('/dashboard', { replace: true, state: { paymentComplete: true } });
      clearCart();
    } catch (err) {
      const apiMsg = err.response?.data?.message;
      showToast(
        (typeof apiMsg === 'string' && apiMsg.trim()) || err.message || 'Checkout failed',
        'error'
      );
    } finally {
      setProcessing(false);
        ...
        <div className="relative">
          ...
          <div className="mt-4 text-left">
            <label htmlFor="transaction-code" className="block text-xs font-semibold uppercase tracking-wide text-gray-400 mb-2">
              Enter transaction code <span className="text-gray-600 normal-case">(optional)</span>
            </label>
            <input
              id="transaction-code"
              type="text"
