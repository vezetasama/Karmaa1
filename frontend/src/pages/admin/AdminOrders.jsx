...
const statusOptions = ['all', 'pending', 'processing', 'completed', 'failed', 'cancelled', 'refunded'];
const statusColors = {
  ...
  completed: { bg: 'bg-neon-green/10', text: 'text-neon-green', border: 'border-neon-green/30' },
...
export default function AdminOrders() {
  ...
  useEffect(() => {
    if (selectedOrder) {
      const code = selectedOrder.giftCardCode || '';
      setCodeVal(code);
      if (selectedOrder.product?.slug === 'netflix') {
        const parts = code.split('|');
        setNetflixEmail(parts[0]?.trim() || '');
        setNetflixPassword(parts[1]?.trim() || '');
      } else {
        setNetflixEmail('');
        setNetflixPassword('');
      }
  ...
  const handleSaveCode = async () => {
    ...
    if (selectedOrder.product?.slug === 'netflix') {
      finalCode = `${netflixEmail.trim()} | ${netflixPassword.trim()}`;
    }

    try {
      await updateOrderStatus(selectedOrder._id, undefined, finalCode);
      setOrders(orders.map(o => o._id === selectedOrder._id ? { ...o, giftCardCode: finalCode } : o));
      setSelectedOrder({ ...selectedOrder, giftCardCode: finalCode });
      setCodeVal(finalCode);
      setSaveSuccess(true);
        ...
        <>
                      ...
                      <tr key={o._id} className="border-b border-white/[0.03] hover:bg-white/[0.02] transition-colors">
                        ...
                        <td className="py-3 px-4">
                          <span className={`inline-flex items-center gap-1 px-2 py-0.5 rounded-full text-[11px] font-medium ${sc.bg} ${sc.text} border ${sc.border}`}>
                            {(o.status === 'completed' || o.status === 'delivered') && <CheckCircle className="w-3 h-3" />}
                        ...
                        <td className="py-3 px-4">
                          ...
                          <button
                            ...
                            className="mt-2 inline-flex items-center gap-1.5 rounded-lg border border-neon-cyan/30 bg-neon-cyan/10 px-2.5 py-1 text-xs font-medium text-neon-cyan hover:bg-neon-cyan/20 transition-all animate-pulse"
                          >
          ...
          <div className="md:hidden space-y-4">
            {orders.map((o) => {
              const sc = statusColors[o.status] || statusColors.pending;
              return (
                <div key={o._id} className="rounded-2xl border border-white/[0.06] bg-dark-800/40 backdrop-blur-sm p-4 space-y-3 relative overflow-hidden">
                  <div className="flex justify-between items-start">
                    <div>
                      <span className="text-xs font-semibold text-neon-cyan bg-neon-cyan/10 px-2 py-0.5 rounded-md border border-neon-cyan/20">{formatOrderId(o)}</span>
                      <p className="text-[10px] text-gray-500 mt-1.5">{new Date(o.createdAt).toLocaleDateString('en-US', { month: 'short', day: 'numeric', hour: '2-digit', minute: '2-digit' })}</p>
                    </div>
                    <span className={`inline-flex items-center gap-1 px-2.5 py-0.5 rounded-full text-[10px] font-bold uppercase tracking-wider ${sc.bg} ${sc.text} border ${sc.border}`}>
                      {(o.status === 'completed' || o.status === 'delivered') && <CheckCircle className="w-3 h-3" />}
                      {o.status === 'pending' && <Clock className="w-3 h-3" />}
                      {o.status === 'processing' && <Loader2 className="w-3 h-3 animate-spin" />}
                      {o.status === 'failed' && <XCircle className="w-3 h-3" />}
                  ...
                  <div className="grid grid-cols-2 gap-y-3 gap-x-2 text-sm bg-dark-900/50 p-3 rounded-xl border border-white/[0.04]">
                    <div>
                      <p className="text-[9px] text-gray-500 uppercase tracking-wide">Customer</p>
                      <p className="font-semibold text-white text-xs mt-0.5 truncate">{o.user?.name || 'N/A'}</p>
                      <p className="text-[9px] text-gray-600 truncate">{o.user?.email}</p>
                    </div>
                    <div>
                      <p className="text-[9px] text-gray-500 uppercase tracking-wide">Product</p>
                      <div className="flex items-center gap-1.5 mt-0.5">
                        ...
                        <span className="text-white text-xs font-semibold truncate">{o.product?.name}</span>
                      </div>
                    </div>
                    <div>
                      <p className="text-[9px] text-gray-500 uppercase tracking-wide">Package</p>
                      <p className="text-neon-cyan text-xs font-semibold mt-0.5 truncate">{o.package?.label}</p>
                    </div>
                    <div>
                  ...
                  <div className="flex items-center gap-2.5 pt-1">
                    <select
                      ...
                      onChange={(e) => handleStatusChange(o._id, e.target.value)}
                      disabled={updating === o._id}
                      className="flex-grow bg-dark-900/60 border border-white/[0.08] rounded-xl px-2.5 py-2 text-xs text-white outline-none focus:border-neon-purple/40 transition-colors"
                    >
                      {['pending', 'processing', 'completed', 'failed', 'cancelled', 'refunded'].map((s) => (
                    ...
                    <button
                      onClick={() => setSelectedOrder(o)}
                      className="inline-flex items-center justify-center gap-1.5 rounded-xl border border-neon-cyan/30 bg-neon-cyan/10 px-3.5 py-2 text-xs font-semibold text-neon-cyan hover:bg-neon-cyan/20 transition-all shrink-0 active:scale-95"
                    >
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
          ...
          <div className="relative w-full max-w-2xl rounded-2xl border border-white/[0.08] bg-dark-900/95 backdrop-blur-xl p-5 sm:p-6 max-h-[85vh] overflow-y-auto">
            ...
            <div className="space-y-5">
              ...
              <div className="rounded-xl border border-white/[0.06] bg-dark-800/50 p-4">
                <p className="text-xs font-semibold uppercase tracking-wide text-gray-400 mb-2.5 flex items-center gap-1.5">
                  <span className="h-1.5 w-1.5 rounded-full bg-neon-purple animate-pulse" />
                  {selectedOrder.product?.slug === 'netflix' ? 'Netflix Account Credentials' : 'Gift Card Delivery Code'}
                </p>
                {selectedOrder.product?.slug === 'netflix' ? (
                  <div className="space-y-3.5">
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-3.5">
                      <div>
                        <label className="block text-[10px] uppercase font-bold text-gray-500 mb-1.5">Netflix Email / ID</label>
                        <input
                          type="text"
                          value={netflixEmail}
                          onChange={(e) => setNetflixEmail(e.target.value)}
                          placeholder="netflix@gmail.com"
                          className="input-field !h-10 !py-2 !px-3.5 text-sm"
                        />
                      </div>
                      <div>
                        <label className="block text-[10px] uppercase font-bold text-gray-500 mb-1.5">Netflix Password</label>
                        <input
                          type="text"
                    ...
                    <div className="flex justify-end">
                      <button
                        ...
                        disabled={savingCode}
                        className={`px-5 py-2.5 rounded-xl font-semibold text-xs transition-all duration-300 flex items-center justify-center gap-1.5 ${
                          saveSuccess
                            ? 'bg-green-600 text-white shadow-[0_0_15px_rgba(34,197,94,0.3)] w-full sm:w-auto'
                            : 'btn-primary !rounded-xl active:scale-95 w-full sm:w-auto'
                        }`}
                      >
                        {savingCode ? (
                          <>
                  ...
                  <div className="flex flex-col sm:flex-row gap-3 items-stretch sm:items-center">
                    <div className="relative flex-grow">
                      <input
                        ...
                        placeholder="Enter code (e.g. AMZN-8392-DK28-9102)"
                        className={`input-field !h-10 !py-2 !px-3.5 text-sm transition-all duration-300 ${
                          saveSuccess ? '!border-green-500/50 !shadow-[0_0_12px_rgba(34,197,94,0.15)]' : ''
                        }`}
                      />
                    </div>
                    <button
                      onClick={handleSaveCode}
                      disabled={savingCode}
                      className={`px-5 py-2.5 rounded-xl font-semibold text-xs transition-all duration-300 flex items-center justify-center gap-1.5 shrink-0 ${
                        saveSuccess
                          ? 'bg-green-600 text-white shadow-[0_0_15px_rgba(34,197,94,0.3)]'
                          : 'btn-primary !rounded-xl active:scale-95'
                      }`}
                    >
                      {savingCode ? (
                        <>
                ...
                <p className="text-[10px] text-gray-600 mt-2">
                  {selectedOrder.product?.slug === 'netflix' 
                    ? "🔒 Credentials are encrypted. Customers can instantly reveal and copy their account email and password from their dashboard." 
                    : "🔒 Input the code manually. Customers can instantly reveal and copy it from their order logs once saved."}
                </p>
              </div>
